...
// ============================================================
// RELAY (Normally Open)
// Set RELAY_PIN to the Arduino pin you wired to IN on the relay module
// ============================================================
const int RELAY_PIN = 8;        // <-- change if needed
const bool RELAY_ON  = HIGH;    // most relay modules: HIGH = ON
...
// ============================================================
// MATRIX FLIP (fix upside-down)
// 180° rotation = reverse row order + reverse bits in each row
// ============================================================
...
void displayText(String txt) {
  ...
  if (txt == "A") { drawImage(letterA); return; }
  if (txt == "B") { drawImage(letterB); return; }
  if (txt == "C") { drawImage(letterC); return; }
  if (txt == ":)" || txt == "SMILE" || txt == "IDLE") { drawImage(smile); return; }
...
// Python sends: "RELAY:ON" or "RELAY:OFF"
// ============================================================
void handleRelayCommand(String data) {
  data.trim();
  data.toUpperCase();
